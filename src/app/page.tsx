...
function Navbar() {
  return (
    <header className="sticky top-0 z-50 border-b border-zinc-800 bg-zinc-950/95 backdrop-blur-sm">
      ...
      <div className="h-0.5 w-full bg-gradient-to-r from-violet-600 via-violet-500 to-violet-700" />
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6">
        ...
        <div className="flex items-center gap-4 overflow-x-auto pb-2 scrollbar-none text-xs text-zinc-500">
          <span className="flex-shrink-0 font-semibold text-zinc-600 uppercase tracking-widest text-[10px]">
            Seções
          </span>
          {categories.map((c) => (
            <a
              key={c.label}
              href="#"
              className="flex-shrink-0 hover:text-zinc-200 transition-colors"
            >
              {c.label}
            </a>
          ))}
    ...
    <article className="group relative overflow-hidden rounded-xl border border-zinc-800 bg-zinc-900 hover:border-violet-700/50 transition-colors cursor-pointer">
      ...
      <div className="relative h-64 sm:h-80 w-full overflow-hidden bg-zinc-800">
        <div
          ...
          style={{
            background:
              "linear-gradient(135deg, #1e1b4b 0%, #312e81 30%, #4c1d95 60%, #1c1c2e 100%)",
          }}
        />
    ...
    <article className="group flex gap-4 py-4 cursor-pointer">
      ...
      <div className="flex-shrink-0 w-24 h-20 rounded-lg overflow-hidden bg-zinc-800">
        <div
          ...
          style={{
            background: `linear-gradient(135deg, hsl(${Math.abs(article.title.charCodeAt(0) * 3) % 360} 40% 18%), hsl(${Math.abs(article.title.charCodeAt(4) * 5) % 360} 50% 25%))`,
          }}
        />
      </div>
      ...
      <div>
        ...
        <ol className="space-y-3">
          {sideArticles.slice(0, 4).map((a, i) => (
            <li key={a.slug} className="group flex items-start gap-3 cursor-pointer">
              <span
    ...
    <div className="min-h-screen bg-zinc-950 text-zinc-100">
      ...
      <main className="mx-auto max-w-7xl px-4 sm:px-6 py-8">
        ...
        <div className="flex items-center gap-3 mb-6 overflow-x-auto">
          <span className="flex-shrink-0 bg-violet-600 text-white text-[10px] font-black uppercase tracking-widest px-2 py-1 rounded">
            Destaque
          </span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="flex items-center gap-3">
              <h2 className="text-xs font-bold uppercase tracking-widest text-zinc-500 flex-shrink-0">
                Artigos recentes
              </h2>
              <Separator className="bg-zinc-800" />
              <a href="#" className="flex-shrink-0 flex items-center gap-1 text-xs text-violet-400 hover:text-violet-300 transition-colors">
            ...
            {/* categorias em cards (mobile-first) */}
            <div>
